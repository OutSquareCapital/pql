...
class TestLazyFrameCreation:
    ...
    def test_scan_table(self) -> None:
        lf = pql.LazyFrame.scan_table("users")
        sql = lf.sql()
...
class TestSelect:
    ...
    def test_select_single_column(self) -> None:
        lf = pql.LazyFrame.scan_table("t").select("x")
        sql = lf.sql().lower()
        assert "select" in sql
        assert "x" in sql

    def test_select_multiple_columns(self) -> None:
        lf = pql.LazyFrame.scan_table("t").select("x", "y", "z")
        sql = lf.sql().lower()
        assert "x" in sql
        assert "y" in sql
        assert "z" in sql
    ...
    def test_select_with_expr(self) -> None:
        lf = pql.LazyFrame.scan_table("t").select(pql.col("x").add(1))
        sql = lf.sql().lower()
        assert "x + 1" in sql or "x+1" in sql

    def test_select_with_alias(self) -> None:
        lf = pql.LazyFrame.scan_table("t").select(
            (pql.col("x").mul(2)).alias("doubled")
        )
        sql = lf.sql().lower()
...
class TestWithColumns:
    ...
    def test_with_columns_adds_column(self) -> None:
        lf = pql.LazyFrame.scan_table("t").with_columns(pql.col("x").alias("x2"))
        sql = lf.sql().lower()
        assert "*" in sql
        assert "x2" in sql

    def test_with_columns_multiple(self) -> None:
        lf = pql.LazyFrame.scan_table("t").with_columns(
...
class TestFilter:
    ...
    def test_filter_simple(self) -> None:
        lf = pql.LazyFrame.scan_table("t").filter(pql.col("x").gt(5))
        sql = lf.sql().lower()
        assert "where" in sql
        assert "x > 5" in sql

    def test_filter_multiple_predicates(self) -> None:
        lf = pql.LazyFrame.scan_table("t").filter(
            pql.col("x").gt(5), pql.col("y").lt(10)
        )
        sql = lf.sql().lower()
        assert "where" in sql
        assert "and" in sql
    ...
    def test_filter_combined_with_and(self) -> None:
        lf = pql.LazyFrame.scan_table("t").filter(
            (pql.col("x").gt(5)).and_(pql.col("y").lt(10))
        )
        sql = lf.sql().lower()
        assert "where" in sql
        assert "and" in sql
    ...
    def test_filter_combined_with_or(self) -> None:
        lf = pql.LazyFrame.scan_table("t").filter(
            (pql.col("x").gt(5)).or_(pql.col("y").lt(10))
        )
        sql = lf.sql().lower()
        assert "where" in sql
        assert "or" in sql
...
class TestSort:
    ...
    def test_sort_single_column(self) -> None:
        lf = pql.LazyFrame.scan_table("t").sort("x")
        sql = lf.sql().lower()
        assert "order by" in sql
        assert "x" in sql

    def test_sort_descending(self) -> None:
        lf = pql.LazyFrame.scan_table("t").sort("x", descending=True)
        sql = lf.sql().lower()
        assert "order by" in sql
        assert "desc" in sql

    def test_sort_multiple_columns(self) -> None:
        lf = pql.LazyFrame.scan_table("t").sort("x", "y", descending=[True, False])
        sql = lf.sql().lower()
        assert "order by" in sql
    ...
    def test_sort_nulls_first(self) -> None:
        ...
        lf = pql.LazyFrame.scan_table("t").sort("x", nulls_last=False)
        sql = lf.sql().lower()
        assert "order by" in sql
    ...
    def test_sort_nulls_last_is_default(self) -> None:
        ...
        lf = pql.LazyFrame.scan_table("t").sort("x", nulls_last=True)
        sql = lf.sql().lower()
        assert "order by" in sql
        assert "x asc" in sql
...
class TestLimit:
    ...
    def test_limit(self) -> None:
        lf = pql.LazyFrame.scan_table("t").limit(10)
        sql = lf.sql().lower()
        assert "limit" in sql
        assert "10" in sql

    def test_head(self) -> None:
        lf = pql.LazyFrame.scan_table("t").head(5)
        sql = lf.sql().lower()
        assert "limit" in sql
        assert "5" in sql
    ...
    def test_tail(self) -> None:
        ...
        lf = pql.LazyFrame.scan_table("t").tail(5)
        sql = lf.sql().lower()
        assert "limit" in sql
...
class TestDistinct:
    ...
    def test_distinct(self) -> None:
        lf = pql.LazyFrame.scan_table("t").distinct()
        sql = lf.sql().lower()
        assert "distinct" in sql

    def test_unique_no_subset(self) -> None:
        lf = pql.LazyFrame.scan_table("t").unique()
        sql = lf.sql().lower()
        assert "distinct" in sql

    def test_unique_with_subset(self) -> None:
        lf = pql.LazyFrame.scan_table("t").unique(subset=["x", "y"])
        sql = lf.sql().lower()
        assert "distinct" in sql
...
class TestJoin:
    ...
    def test_inner_join_on(self) -> None:
        lf1 = pql.LazyFrame.scan_table("t1")
        lf2 = pql.LazyFrame.scan_table("t2")
...
class TestChaining:
    ...
    def test_filter_then_select(self) -> None:
        lf = (
            pql.LazyFrame.scan_table("users")
    ...
    def test_full_pipeline(self) -> None:
        lf = (
            pql.LazyFrame.scan_table("orders")
...
class TestSqlOutput:
    ...
    def test_sql_pretty(self) -> None:
        lf = pql.LazyFrame.scan_table("t").filter(pql.col("x").gt(5)).select("x", "y")
